...
class TestFederatedSparql(unittest.TestCase):
    ...
    def setUp(self):
        import os
        import uvicorn
        # Start the mapping service endpoint in the background
        converter = Converter.from_priority_prefix_map(PREFIX_MAP)
        app = get_fastapi_mapping_app(converter)

        # Download blazegraph.jar if not present
        if not os.path.exists("blazegraph.jar"):
            subprocess.run("wget -N https://github.com/blazegraph/database/releases/download/BLAZEGRAPH_2_1_6_RC/blazegraph.jar", shell=True)
    ...
    def test_federated_local_blazegraph(self):
        ...
        try:
            ...
            self.assertTrue(
                False, msg=f"Error running the federated query to {BLAZEGRAPH_ENDPOINT}: {resp.text}"
            )
        return None
